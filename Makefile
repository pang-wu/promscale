...
.PHONY: build
build: generate
	export GOOS=linux GOARCH=amd64 && go build -ldflags "-X 'github.com/timescale/promscale/pkg/version.Branch=${GIT_BRANCH}' -X 'github.com/timescale/promscale/pkg/version.CommitHash=${GIT_COMMIT}'" -o dist/promscale ./cmd/promscale
...
.PHONY: e2e
e2e: DOCKER_IMAGE?=$(shell ./scripts/fallback-docker.sh $(LOCAL_DOCKER_BASE):head-ts2.8.1-pg14 $(GHCR_DOCKER_BASE):$(CURRENT_BRANCH)-ts2.8.1-pg14 $(GHCR_DOCKER_BASE):$(EXTENSION_VERSION)-ts2.8.1-pg14)
...
jaeger-storage-test: DOCKER_IMAGE?=$(shell ./scripts/fallback-docker.sh $(LOCAL_DOCKER_BASE):head-ts2.8.1-pg14 $(GHCR_DOCKER_BASE):$(CURRENT_BRANCH)-ts2.8.1-pg14 $(GHCR_DOCKER_BASE):$(EXTENSION_VERSION)-ts2.8.1-pg14)
jaeger-storage-test:
...
.PHONY: upgrade-test
upgrade-test: DOCKER_IMAGE?=$(shell ./scripts/fallback-docker.sh $(LOCAL_DOCKER_BASE):head-ts2.8.1-pg13 $(GHCR_DOCKER_BASE):$(CURRENT_BRANCH)-ts2.8.1-pg13 $(GHCR_DOCKER_BASE):$(EXTENSION_VERSION)-ts2.8.1-pg13)
upgrade-test:
